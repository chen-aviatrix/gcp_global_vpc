...
# --------------------------------------------------------------------------- #
# VMs in vpc-us2: 2 per region (us-central1, us-west2, us-south1)
# --------------------------------------------------------------------------- #

module "gcp_vpc_us2_central1_vm1" {
  source               = "../mc-vm-csp/gcp"
  resource_name_label  = "${local.prefix}-us2-central1-vm1"
  region               = aviatrix_vpc.vpc_us2.subnets[0].region
  region2              = aviatrix_vpc.vpc_us2.subnets[0].region
  vpc_id               = aviatrix_vpc.vpc_us2.vpc_id
  public_subnet_id     = aviatrix_vpc.vpc_us2.subnets[0].name
  private_subnet_id    = aviatrix_vpc.vpc_us2.subnets[0].name
  ingress_cidrs        = var.ingress_cidrs # List of CIDRs to allow ingress traffic thru SSH/ICMP to the VMs
  use_existing_keypair = true
  public_key           = tls_private_key.terraform_key.public_key_openssh
}
...
module "gcp_vpc_us2_central1_vm2" {
  ...
  region               = aviatrix_vpc.vpc_us2.subnets[1].region
  region2              = aviatrix_vpc.vpc_us2.subnets[1].region
  vpc_id               = aviatrix_vpc.vpc_us2.vpc_id
  public_subnet_id     = aviatrix_vpc.vpc_us2.subnets[1].name
  private_subnet_id    = aviatrix_vpc.vpc_us2.subnets[1].name
  ingress_cidrs        = var.ingress_cidrs # List of CIDRs to allow ingress traffic thru SSH/ICMP to the VMs
  use_existing_keypair = true
  public_key           = tls_private_key.terraform_key.public_key_openssh
}
...
module "gcp_vpc_us2_west2_vm1" {
  ...
  region               = aviatrix_vpc.vpc_us2.subnets[4].region
  region2              = aviatrix_vpc.vpc_us2.subnets[4].region
  vpc_id               = aviatrix_vpc.vpc_us2.vpc_id
  public_subnet_id     = aviatrix_vpc.vpc_us2.subnets[4].name
  private_subnet_id    = aviatrix_vpc.vpc_us2.subnets[4].name
  ingress_cidrs        = var.ingress_cidrs # List of CIDRs to allow ingress traffic thru SSH/ICMP to the VMs
  use_existing_keypair = true
  public_key           = tls_private_key.terraform_key.public_key_openssh
}
...
module "gcp_vpc_us2_west2_vm2" {
  ...
  region               = aviatrix_vpc.vpc_us2.subnets[5].region
  region2              = aviatrix_vpc.vpc_us2.subnets[5].region
  vpc_id               = aviatrix_vpc.vpc_us2.vpc_id
  public_subnet_id     = aviatrix_vpc.vpc_us2.subnets[5].name
  private_subnet_id    = aviatrix_vpc.vpc_us2.subnets[5].name
  ingress_cidrs        = var.ingress_cidrs # List of CIDRs to allow ingress traffic thru SSH/ICMP to the VMs
  use_existing_keypair = true
  public_key           = tls_private_key.terraform_key.public_key_openssh
}
...
module "gcp_vpc_us2_south1_vm1" {
  ...
  region               = aviatrix_vpc.vpc_us2.subnets[8].region
  region2              = aviatrix_vpc.vpc_us2.subnets[8].region
  vpc_id               = aviatrix_vpc.vpc_us2.vpc_id
  public_subnet_id     = aviatrix_vpc.vpc_us2.subnets[8].name
  private_subnet_id    = aviatrix_vpc.vpc_us2.subnets[8].name
  ingress_cidrs        = var.ingress_cidrs # List of CIDRs to allow ingress traffic thru SSH/ICMP to the VMs
  use_existing_keypair = true
  public_key           = tls_private_key.terraform_key.public_key_openssh
}
...
module "gcp_vpc_us2_south1_vm2" {
  ...
  region               = aviatrix_vpc.vpc_us2.subnets[9].region
  region2              = aviatrix_vpc.vpc_us2.subnets[9].region
  vpc_id               = aviatrix_vpc.vpc_us2.vpc_id
  public_subnet_id     = aviatrix_vpc.vpc_us2.subnets[9].name
  private_subnet_id    = aviatrix_vpc.vpc_us2.subnets[9].name
  ingress_cidrs        = var.ingress_cidrs # List of CIDRs to allow ingress traffic thru SSH/ICMP to the VMs
  use_existing_keypair = true
  public_key           = tls_private_key.terraform_key.public_key_openssh
}
